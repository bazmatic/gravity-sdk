...
impl Mempool {
    ...
    pub async fn add_verified_txn(&self, txn: VerifiedTxn) -> TxnHash {
    ...
    pub async fn add_raw_txn(&self, bytes: Vec<u8>) -> TxnHash {
        let raw_txn = TransactionWithAccount::from_bytes(bytes);
