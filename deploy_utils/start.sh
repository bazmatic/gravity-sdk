...
function start_node() {
    ...
    pid=$(${ROOT_PATH}/target/debug/gravity_consensus --node-config-path ${node_path}/genesis/validator.yaml --mockdb-config-path ${ROOT_PATH}/nodes_config.json > ${node_path}/logs/debug.log & echo $!)
