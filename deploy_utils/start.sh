...
function start_node() {
    ...
    pid=$(${ROOT_PATH}/target/debug/gravity_consensus --data-path ${cur_node_path}/data --port ${port}  > ${node_path}/logs/debug.log & echo $!)
...
if [ ! -d ${ROOT_PATH}/${node_arg}/config ]; then
    mkdir ${ROOT_PATH}/${node_arg}/config
    cp ${ROOT_PATH}/template_config/* ${ROOT_PATH}/${node_arg}/config/
fi
